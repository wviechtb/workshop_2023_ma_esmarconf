...
plot(sav, lwd=c(2,6,6,4), col.ref="red")
...
# for rma.mv() models, can use test="t", which also uses t-distributions for
# the tests (but without an adjustment to the SEs); by default, the dfs are
# computed with k-p, but slightly better is dfs="contain" (see help(rma.mv)
# for details)
...
# simulation approach to examine whether the (G)VIFs are large
sav <- vif(res, btt=c("grade", "length", "meta"), sim=TRUE, seed=1234)
sav

# plot the simulated (G)VIFs and compare against the observed ones
plot(sav, breaks=seq(1,2.5,by=.0125), xlim=c(1,1.8))
