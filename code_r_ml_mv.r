...
# subgrouping: assuming equal tau^2 within subgroups versus allowing tau^2 to
...
# mixed-effects meta-regression model with a categorical variable
res <- rma(yi, vi, mods = ~ 0 + alloc, data=dat)
res

# fit same model using rma.mv()
res0 <- rma.mv(yi, vi, mods = ~ 0 + alloc,
               random = ~ 1 | trial, data=dat)
res0

# allow tau^2 to differ across the levels of the alloc variable
res1 <- rma.mv(yi, vi, mods = ~ 0 + alloc,
               random = ~ alloc | trial, struct="DIAG", data=dat)
res1

# same as fitting separate random-effects models in each subgroup
res <- list(rma(yi, vi, data=dat, subset=alloc=="alternate"),
            rma(yi, vi, data=dat, subset=alloc=="random"),
            rma(yi, vi, data=dat, subset=alloc=="systematic"))

tab <- data.frame(tau2     = sapply(res, \(x) x$tau2),
                  estimate = sapply(res, coef),
                  se       = sqrt(sapply(res, vcov)))
rownames(tab) <- c("alternate", "random", "systematic")
round(tab, digits=4)
...
# test if the type of delinquent behavior is a potential moderator
